...
Authentication must be implemented using **better-auth** with the email/password method.
...
**Session access in server components / API routes**:
...
**Database migration** — run once to create better-auth tables:
...
* `account` — id, userId, accountId, providerId, password, accessToken, refreshToken, etc.
* `verification` — id, identifier, value, expiresAt, createdAt, updatedAt

Do **not** create these tables manually.
...
app/
  ...
  api/
    auth/
      [...all]/          ← better-auth catch-all handler (toNextJsHandler)
        route.ts
    notes/
      route.ts
      [noteId]/
...
lib/
  auth.ts          ← betterAuth() server instance (bun:sqlite)
  auth-client.ts   ← createAuthClient() for React/client components
  db.ts            ← Bun SQLite connection for notes
...
## 8.1 Auth Tables (auto-generated by better-auth)

Better-auth automatically creates and manages these tables via `bunx --bun auth@latest migrate`. **Do not create them manually.**
...
The `user.id` field is a string (TEXT) and is used as `user_id` in the notes table.
...
* Install `better-auth` and run `bunx --bun auth@latest migrate` to create auth tables.
* Create `lib/auth.ts` with `betterAuth({ database: new Database("./sqlite.db"), emailAndPassword: { enabled: true } })`.
* Create `app/api/auth/[...all]/route.ts` using `toNextJsHandler(auth)` from `better-auth/next-js`.
* Create `lib/auth-client.ts` using `createAuthClient()` from `better-auth/react`.
* Create `middleware.ts` to protect `/notes/*` routes using `auth.api.getSession()`.
* Add login and registration pages using `authClient.signIn.email()` and `authClient.signUp.email()`.
* Add logout using `authClient.signOut()`.
* Protect API routes using `auth.api.getSession({ headers: await headers() })`.
